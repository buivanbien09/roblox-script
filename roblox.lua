...
task.spawn(function()
    ...
    local imgUrl = "https://github.com/buivanbien09/bienxinloi09/blob/main/ok.png?raw=true"
    local imgName = "fluent_toggle_icon_v3.png"
    local customAssetId = ""

    pcall(function()
        if isfile and writefile and getcustomasset then
            if not isfile(imgName) then
                writefile(imgName, game:HttpGet(imgUrl))
            end
            customAssetId = getcustomasset(imgName)
        end
    end)

    local ToggleGui = Instance.new("ScreenGui")
    ToggleGui.Name = "MenuToggleGui"
    ...
    ToggleBtn.Image = customAssetId ~= "" and customAssetId or "rbxassetid://10618928818" -- Ảnh mặc định nếu executor không hỗ trợ tải ảnh
